...
RANDOM_STATE = 13
ATTENTION_HIDDEN_RATIO = 4  # hidden_size // ratio
N_SPLITS = 4
...
def train_one_fold(
    ...
    """Train with early stopping on validation AP (maximize), scheduler on val loss."""
    # Data
    ...
    # Optim & scheduler (scheduler still follows val loss)
    ...
    best_state = None
    patience = 0
    best_val_ap = float("-inf")   # <-- monitor AP (maximize)
    eps = 1e-6
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        with torch.no_grad():
            for xb, yb in val_loader:
                ...
                logits = model(xb)
                # loss for scheduler (stable, differentiable)
        ...
        # AP can fail if val set is single-class; guard and fallback
        try:
            val_ap = average_precision_score(y_true, y_proba)
        except Exception:
            val_ap = float("nan")

        # Keep LR scheduler tied to val loss
        if scheduler is not None:
            scheduler.step(val_loss)

        # -------- Early stopping on AP (maximize) --------
        improved = (not np.isnan(val_ap)) and (val_ap > best_val_ap + eps)
        if improved:
            best_val_ap = val_ap
            best_epoch = epoch
        ...
        if epoch % 10 == 0 or epoch == 1:
            ...
            print(f"Epoch {epoch:04d} | Train {train_loss:.4f} | ValLoss {val_loss:.4f} | ValAP {ap_txt}")

        if patience >= EARLY_STOP_PATIENCE:
            # If AP was never valid (extreme class imbalance in fold), fall back to last state
            if best_state is None:
                best_state = deepcopy(model.state_dict())
                best_epoch = epoch
            break

    # Restore best (by AP) and evaluate fold metrics for reporting
    model.load_state_dict(best_state)
    fold_metrics = evaluate(model, val_loader, device, threshold=THRESHOLD)
    # Also include the best AP we monitored (useful in CSV)
    fold_metrics["early_stop_ap"] = float(best_val_ap)
...
# ============================================================
print("\n" + "=" * 60)
print(f"PHASE 2: Training final model on 100% data for {optimal_epochs} epochs")
print("=" * 60)
...
if USE_CLASS_WEIGHTS:
    pos_weight_full = (y == 0).sum() / (y == 1).sum()
    criterion_full = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([pos_weight_full])).to(device)
else:
    criterion_full = nn.BCEWithLogitsLoss().to(device)
...
# Train for optimal number of epochs on full data
for epoch in range(1, optimal_epochs + 1):
    ...
    for xb, yb in full_loader:
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(final_model.parameters(), max_norm=GRADIENT_CLIP)
        optimizer_full.step()
    ...
    if epoch % 5 == 0 or epoch == 1 or epoch == optimal_epochs:
        print(f"Epoch {epoch:02d}/{optimal_epochs} — Loss: {avg_loss:.4f}")
...
print(f"Train-Val gap at best epoch: {train_losses[best_epoch-1] - best_val_loss:.4f}")
